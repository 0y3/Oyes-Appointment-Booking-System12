...
const submitBooking = async (values, { resetForm }) => {
  ...
  loading.value = true
  try {
    const datetimeString = `${values.date} ${values.slot}`
      ...
      <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="w-full md:w-1/2 space-y-4">
          ...
          <VDatePicker
            ...
            @update:model-value="val => { setFieldValue('date', val); setFieldValue('slot', ''); selectedSlot = null }"
          >
            <template #default="{ inputValue, inputEvents }">
              <div class="flex rounded-lg border border-gray-300 overflow-hidden mb-1 bg-white">
                ...
                <input
                  ...
                  required
                  name="date"
                  @input="setFieldValue('date', inputValue)"
                  @blur="setFieldValue('date', inputValue)"
                />
              </div>
            </template>
          </VDatePicker>
        ...
        <!-- Time Slots -->
        <!-- <div  class="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-30">
            <div class="flex flex-col items-center">
                ...
                <span class="text-white text-lg font-semibold">Loading slots...</span>
            </div>
        </div> -->
        ...
        <div v-if="availableSlots.length" class="w-full md:w-1/2 flex flex-col h-full">
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 max-h-64 overflow-y-auto pr-2">
            <button
              ...
              :key="slot.start"
              type="button"
              @click="() => { selectSlot(slot); setFieldValue('slot', moment(slot.start, 'HH:mm').format('h:mm a')) }"
              :class="[
          ...
          <div class="mb-6">
            <Field name="slot" type="hidden" />
            <ErrorMessage name="slot" class="text-red-500 text-xs" />
          </div>
          <button
            type="submit"
            :disabled="loading"
